...
\begin{frame}[t]{Balance theory}{A brief description}
	...
	\onslide<-6>{
	\begin{itemize}[<+(+2)->]
		\item If all three people are friend, there \textbf{is balance} 
		\item If all three people are enemies, there \textbf{no balance} 
		\item If two people are enemies from the third, there \textbf{is balance} \item If only two people are enemies, there is \textbf{no balance} 
\end{itemize}}
	\end{minipage}
	\foreach \n in {1,...,7}{
