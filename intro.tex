...
\begin{frame}{Context}
	\centering
	\includesvg[inkscapearea=page,width=0.8\columnwidth]{dip}

	\onslide<+->{We wanted to study the Chilean Chamber of Deputies and its (political) polarization.\\[1em]}

	In order to accomplish that, we built a model using tools from \textbf<+->{graph theory} and \textbf<+->{statistical mechanics}.
	
\vspace{2em}
\onslide<+->{Parliament} \onslide<+->{$\to$ Network} \onslide<+>{$\to$ Spin systems}
...
\begin{frame}[b]{Graph theory}{What is a graph?}
	...
	\begin{itemize}
		\only<+(-3)->{\item Nodes and links}\only<+(-3)->{, that may have direction}\only<+(-3)->{, and weights}
	\end{itemize}
	\vspace{2em}
\end{frame}
...
\foreach \n in {1,...,4}{
	\ifnum\n =2 
	\else
	\only<+(-3)>{\fullsizegraphic{complex-step-\n.pdf}}
	\fi
}
\end{frame}

\begin{frame}[t]{Social networks}{}
	\only<+>{\vspace{10em}\centering In these networks nodes represent people.}	
	\begin{itemize}[<+->]
...
\begin{frame}{Legislative networks}{A specific type of political networks}
	\only<+>{\vspace{10em}\centering In these networks nodes represent legislators.}
	\foreach \n in {1,...,6}{
		        \only<+>{\fullsizegraphic{papers-leg-step-\n.pdf}}
	}
	\only<+->{\fullsizegraphic{papers-leg-step-7.pdf}}
	\vspace{5cm}
	\begin{itemize}[<+->]
...
\begin{frame}[t]{Balance theory}{A brief description}
	...
	\onslide<-6>{
	\begin{itemize}[<+(+2)->]
		\item If all three people are friend, there is \textbf{is balance} 
		\item If all three people are enemies, there is \textbf{no balance} 
		\item If two people are enemies from the third, there is \textbf{balance} \item If only two people are enemies, there is \textbf{no balance} 
\end{itemize}}
	\end{minipage}
	\foreach \n in {1,...,7}{
